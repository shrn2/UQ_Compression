"""JSONL data contract used by calibration and evaluation commands."""
...
def load_jsonl(path: str | Path, *, limit: int | None = None) -> list[MultipleChoiceExample]:
    """Load examples with fields: id, prompt, choices, and optional answer/dataset."""
        ...
        for line_number, line in enumerate(handle, start=1):
            ...
            try:
                ...
                if len(choices) < 2 or not str(item["prompt"]).strip():
                    raise ValueError("prompt must be non-empty and choices must contain at least two items")
                if answer is not None and (not isinstance(answer, int) or not 0 <= answer < len(choices)):
...
def pad_probabilities(rows: Iterable[Iterable[float]], width: int | None = None):
    """Pad variable-choice distributions with zeros for vectorized metrics."""
